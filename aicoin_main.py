...
class PieChart(QMainWindow):
    ...
    def Draw(self, tbl, upbit):
        ...
        if ptmp < 0:
            self.chart.setTitle(
                f'<span style="color:blue;">투자액: {tmp} 원  잔액: {btmp}<br>수익율:   {pratio} %</span>'
            )
        else:
            self.chart.setTitle(
                f'<span style="color:red;">투자액: {tmp} 원 잔액: {btmp}<br>수익율:   {pratio} %</span>'
            )
...
class LoginDlg(QDialog):
    ...
    def setStyle(self):
        ...
        self.ui.pushButton_ok.clicked.connect(
            lambda x: self.btn_event(self.ui.pushButton_ok)
        )
        self.ui.pushButton_exit.clicked.connect(
            lambda x: self.btn_event(self.ui.pushButton_exit)
        )
...
class MainWindow(QMainWindow):
    ...
    def set_Combo(self):
        combo_boxes = [
            self.ui.comboBox_buyPratio,
            self.ui.comboBox_buyAratio,
            self.ui.comboBox_sellAratio,
            self.ui.comboBox_sellPratio,
            self.ui.comboBox_asellrate,
            self.ui.comboBox_spsellrate,
            self.ui.comboBox_abuyrate,
            self.ui.comboBox_aspbuyrate,
        ]

        for combo_box in combo_boxes:
            combo_box.setStyleSheet(cmbstyle)
            combo_box.setEditable(True)
            combo_box.lineEdit().setAlignment(QtCore.Qt.AlignCenter)
            combo_box.view().setVerticalScrollBarPolicy(Qt.ScrollBarAsNeeded)
    ...
    def get_orderlist(self):
        ...
        logging.info(data)
        for jdata in data:
            # Process each order data here
            row = self.ui.tableWidget_tradelist.rowCount()
    ...
    def set_tblevt(self):
        self.Chart = PieChart()
        self.ui.tableWidget_status.doubleClicked.connect(
            lambda x: self.tbl_doubleClicked(self.ui.tableWidget_status)
        )
        self.ui.tableWidget_status.clicked.connect(
            lambda x: self.tbl_clicked(self.ui.tableWidget_status)
        )
        self.ui.tableWidget_tot.clicked.connect(
            lambda x: self.tbl_clicked(self.ui.tableWidget_tot)
        )
        self.ui.treeWidget_autolist.clicked.connect(
            lambda x: self.tbl_clicked(self.ui.treeWidget_autolist)
        )
        self.ui.treeWidget_autolist.doubleClicked.connect(
            lambda x: self.tbl_doubleClicked(self.ui.treeWidget_autolist)
        )
        self.ui.tabWidget_main.currentChanged.connect(
            lambda x: self.tabSelected(
                self.ui.tabWidget_main, self.ui.tabWidget_main.currentIndex()
            )
        )

        self.ui.tabWidget_main.setCurrentIndex(0)
        self.ui.tabWidget_sub.setCurrentIndex(0)
        self.ui.comboBox_buytype.currentIndexChanged.connect(
            lambda x: self.setRatio(self.ui.comboBox_buytype)
        )
        self.ui.comboBox_buyPratio.currentIndexChanged.connect(
            lambda x: self.setRatio(self.ui.comboBox_buyPratio)
        )
        self.ui.comboBox_buyAratio.currentIndexChanged.connect(
            lambda x: self.setRatio(self.ui.comboBox_buyAratio)
        )

        self.ui.doubleSpinBox_Abuy.valueChanged.connect(
            lambda x: self.set_ValueChanged(self.ui.doubleSpinBox_Abuy)
        )
        self.ui.doubleSpinBox_Pbuy.valueChanged.connect(
            lambda x: self.set_ValueChanged(self.ui.doubleSpinBox_Pbuy)
        )
    ...
    def set_btnevt(self):
        self.ui.pushButton_start.clicked.connect(
            lambda x: self.btn_event(self.ui.pushButton_start)
        )
        self.ui.pushButton_stop.clicked.connect(
            lambda x: self.btn_event(self.ui.pushButton_stop)
        )
        self.ui.pushButton_close.clicked.connect(
            lambda x: self.btn_event(self.ui.pushButton_close)
        )
        self.ui.pushButton_reflash.clicked.connect(
            lambda x: self.btn_event(self.ui.pushButton_reflash)
        )
        self.ui.pushButton_update.clicked.connect(
            lambda x: self.btn_event(self.ui.pushButton_update)
        )
        self.ui.pushButton_cancel.clicked.connect(
            lambda x: self.btn_event(self.ui.pushButton_cancel)
        )

        self.ui.radioButton_krw.clicked.connect(
            lambda x: self.market_list(self.ui.radioButton_krw)
        )
        self.ui.radioButton_fav.clicked.connect(
            lambda x: self.market_list(self.ui.radioButton_fav)
        )
        self.set_autolist()
    ...
    def setbalance(self, data):
        ...
        if cname == "KRW" and avg == "0":
            ...
            tcol0.setFlags(tcol0.flags() & ~(Qt.ItemIsEditable))
            tcol0.setFlags(tcol0.flags() | (Qt.ItemIsSelectable))
            ...
            tcol3.setFlags(tcol3.flags() & ~(Qt.ItemIsEditable))
            tcol3.setFlags(tcol3.flags() | (Qt.ItemIsSelectable))
            ...
            tcol4.setFlags(tcol4.flags() & ~(Qt.ItemIsEditable))
            tcol4.setFlags(tcol4.flags() | (Qt.ItemIsSelectable))
    ...
    def setTradeStatus(self, data):
        ...
        col5 = QTableWidgetItem("매수" if tmp == "ASK" else "매도")
        col5.setTextAlignment(Qt.AlignCenter | Qt.AlignVCenter)
        col5.setFlags(col5.flags() & ~(Qt.ItemIsEditable))
        col5.setFlags(col5.flags() | (Qt.ItemIsSelectable))
        self.ui.tableWidget_tradesum.setItem(self.tradesum_row, 5, col5)

        col6 = QTableWidgetItem(str(jdata.get("sequential_id")))
    ...
    def setwatch(self, data):
        ...
        self.ui.statusBar().showMessage(
            self.date.toString(Qt.ISODateWithMs) + " " + data
        )
    ...
    def set_tbleReszie(self):
        self.ui.tableWidget_status.horizontalHeader().setSectionResizeMode(
            QHeaderView.Stretch
        )
        self.ui.tableWidget_tradesum.horizontalHeader().setSectionResizeMode(
            QHeaderView.Stretch
        )
        self.ui.tableWidget_tot.horizontalHeader().setSectionResizeMode(
            QHeaderView.Stretch
        )
        self.ui.tableWidget_tradelist.horizontalHeader().setSectionResizeMode(
            QHeaderView.Stretch
        )
        self.ui.tableWidget_balance.horizontalHeader().setSectionResizeMode(
            QHeaderView.Stretch
        )
        self.ui.treeWidget_autolist.header().setSectionResizeMode(
